# Patrick Salvador Hernández Arias 🚀
## Analista de Datos | Estudiante de Contaduría Pública

---

### 👤 Perfil Profesional
Estudiante de Contaduría Pública con especialización en análisis de datos. Mi enfoque combina la **precisión financiera** con la **analítica avanzada**, aplicando estándares internacionales de comunicación (IBCS) para transformar datos complejos en decisiones estratégicas. Destaco por mi pensamiento analítico y aprendizaje autodidacta.

---

### 🎓 Formación Académica
* **Licenciatura en Contaduría Pública** | UTEL | *En curso* 🏛️
* **Data Analyst** | EBAC | *2025 - 2026* 📊

---

### 📈 Proyectos Destacados (EBAC)
* **Análisis y Visualización de Datos** | *2025*
    * 🧹 **Limpieza:** Procesamiento de datos en Excel para asegurar integridad.
    * 🗄️ **SQL:** Gestión y estructuración de bases de datos relacionales.
    * 🤖 **Machine Learning:** Modelos de predicción de ventas utilizando Python y PyCaret.
    * ☁️ **Cloud & BI:** Integración con AWS y dashboards interactivos en Looker Studio.
    * 💻 **Control:** Versionado de código y gestión de proyectos mediante GitHub.

---

### 💼 Experiencia Profesional
**Ley Alica** | *Tepic, Nayarit* | *2025 - Actualidad* 🛒
* Atención y soporte al cliente en entornos de alta demanda.
* Optimización de procesos operativos mediante el uso de herramientas digitales.
* Resolución de problemas en tiempo real y trabajo colaborativo.

---

### 🛠️ Competencias Técnicas

<table>
  <tr>
    <td align="center" width="150">
      <img src="https://img.shields.io/badge/Excel-217346?style=for-the-badge&logo=microsoft-excel&logoColor=white" width="140" />
    </td>
    <td align="center" width="150">
      <img src="https://img.shields.io/badge/Word-2B579A?style=for-the-badge&logo=microsoft-word&logoColor=white" width="140" />
    </td>
    <td align="center" width="150">
      <img src="https://img.shields.io/badge/PowerPoint-B7472A?style=for-the-badge&logo=microsoft-powerpoint&logoColor=white" width="140" />
    </td>
    <td align="center" width="150">
      <img src="https://img.shields.io/badge/SQL_Server-CC2927?style=for-the-badge&logo=microsoft-sql-server&logoColor=white" width="140" />
    </td>
  </tr>
  <tr>
    <td align="center" width="150">
      <img src="https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white" width="140" />
    </td>
    <td align="center" width="150">
      <img src="https://img.shields.io/badge/Pandas-150458?style=for-the-badge&logo=pandas&logoColor=white" width="140" />
    </td>
    <td align="center" width="150">
      <img src="https://img.shields.io/badge/Looker_Studio-4285F4?style=for-the-badge&logo=looker&logoColor=white" width="140" />
    </td>
    <td align="center" width="150">
      <img src="https://img.shields.io/badge/AWS-232F3E?style=for-the-badge&logo=amazonaws&logoColor=white" width="140" />
    </td>
  </tr>
</table>

---

### 💡 Habilidades Blandas & Idiomas
* 🧠 **Pensamiento analítico** | 🤝 **Trabajo en equipo** | 🗣️ **Comunicación IBCS**
* 🌐 **Inglés:** Nivel B1

---

### 📬 Contacto
* 📧 **Email:** [Patrick.hernandez.arias@gmail.com](mailto:Patrick.hernandez.arias@gmail.com)
* 📱 **Teléfono:** +52 311-877-6364
* 🔗 **LinkedIn:** [Tu URL aquí]
* 🐙 **GitHub:** [Tu URL aquí]

---
*Construyendo soluciones basadas en datos.*
